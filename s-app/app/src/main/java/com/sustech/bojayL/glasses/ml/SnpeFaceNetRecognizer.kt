...
/**
 * FaceNet-MobileNetV2 (256d) INT8 SNPE 特征提取器。
 *
 * 说明：
 * - 主路径：SNPE + DLC 模型（`facenet_mobilenetv2_256_int8.dlc`）。
 * - 若设备未集成 SNPE Runtime 或模型缺失，返回 null，由上层回退到 NCNN。
 */
class SnpeFaceNetRecognizer {
    ...
    private var modelAssetName: String? = null
    ...
    fun init(context: Context): Boolean {
        ...
        return try {
            ...
            val builderObj = createBuilder(context) ?: run {
                Log.e(TAG, "Failed to create SNPE builder")
                return false
            }

            if (!configureBuilder(builderObj, context, copiedModel, asset)) {
                Log.e(TAG, "Failed to configure SNPE builder")
                return false
            }

            val buildResult = invokeReflect(builderObj, "build")
            ...
            modelAssetName = asset
            modelFile = copiedModel
            initialized = true
    ...
    fun extractFeature(alignedFace112: Bitmap): FloatArray? {
        ...
        return try {
            ...
            EmbeddingUtils.toNormalized256(raw)
        } catch (e: Throwable) {
